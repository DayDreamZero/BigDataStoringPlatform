  ...
  renderOper = (value, index, record) => {
    return (
      <div style={styles.oper}>
        ...
        >权限</Link>
        <Link 
        to={{pathname:`/userManagement/auditSubUserOperation`}} 
        onClick={(e)=>{localStorage.setItem('subUserId',record.userId); localStorage.setItem('subUserName',record.userName);e.stopPropagation();}} 
        style={{display:'inline-block',marginRight:'10px'}}
        >审计</Link>
      </div>
    );
